...
raw_data <- read_csv("AAKmaTow.csv")
...
gss <- gss %>% 
  ...
  mutate(education = case_when(
    education=="Less than high school diploma or its equivalent" ~ 0,
    education=="High school diploma or a high school equivalency certificate" ~ 1,
    ...
    education=="University certificate, diploma or degree above the bach..." ~ 4,
    education=="NA" ~ 5
  ))
...
gss2$age <- as.integer(gss2$age)


write_csv(gss2, "final.csv")
